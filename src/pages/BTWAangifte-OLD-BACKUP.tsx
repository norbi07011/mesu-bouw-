...
import { useKilometers } from '../hooks/useElectronDB';
...
export function BTWAangifte() {
  ...
  // Calculate BTW data from invoices, expenses, and kilometers for the selected period
  const calculatedData: BTWCalculationData = useMemo(() => {
    ...
    // Filter kilometers for the selected period
    const periodKilometers = kilometers.filter((km: any) => {
      return km.date >= startDate && km.date <= endDate;
    });

    // Calculate private use from kilometers (if using private vehicle for business)
    // In Netherlands, if you use private car for business, you may need to add BTW for private use
    const privateUseKm = periodKilometers.reduce((sum: number, km: any) => {
      // Only count if it's a private vehicle used for business
    ...
    // Private use VAT (21% of private use amount) - applicable if you deduct BTW on car expenses
    const privateUseVat = privateUseKm * 0.21;

    const vatToPay = vat21 * 0.21 + vat9 * 0.09 + privateUseVat;
    ...
    return {
      ...
      kilometers: {
        total: periodKilometers.length,
        privateUse: privateUseKm,
        privateUseVat,
      },
      balance,
    };
  ...
  const handleExportPDF = async (declaration: any) => {
    try {
      if (!company) {
        toast.error(t('btw.errorNoCompany'));
        return;
      }
      await generateBTWDeclarationPDF(declaration, company, t('btw.language'));
  ...
  if (btwLoading || invoicesLoading || expensesLoading || kilometersLoading) {
    return (
        ...
        <Card className="mb-6 p-6">
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card className="p-4 bg-orange-50 border-orange-200">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600">Privégebruik (km)</p>
                  <p className="text-2xl font-bold text-orange-900">
                    {calculatedData.kilometers?.total || 0} trips
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    BTW: {formatCurrency(calculatedData.kilometers?.privateUseVat || 0)}
                  </p>
                </div>
