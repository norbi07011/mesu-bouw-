...
function Card({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "relative bg-white/80 backdrop-blur-sm border border-white/30 text-gray-900 flex flex-col gap-6 rounded-2xl py-6 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.02]",
        className
      )}
      {...props}
    />
