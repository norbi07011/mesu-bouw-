...
class DatabaseService {
    ...
    `);
        // Dodaj kolumnę language do istniejących tabel (migracja)
        try {
            this.db.exec(`ALTER TABLE invoices ADD COLUMN language TEXT DEFAULT 'nl'`);
        }
        catch (e) {
    ...
    `);
      ...
      CREATE TABLE IF NOT EXISTS expenses (
        ...
        is_vat_deductible INTEGER DEFAULT 1,
        is_business_expense INTEGER DEFAULT 1,
        private_percentage REAL DEFAULT 0,
        project_id TEXT,
        client_id TEXT,
        invoice_number TEXT,
        receipt_url TEXT,
    ...
    seedDefaultData() {
      ...
      `).run(companyId, 'NORBS SERVICE', 'Amsterdam, Netherlands', '94061629', 'NL005061645B57', 'NL25INGB0109126122', 'INGBNL2A', 'info@norbsservice.nl', 'ING Bank', 'NL25INGB0109126122', now, now);
        ...
        // Migracja: ustaw domyślny język dla istniejących faktur
        try {
            this.db.prepare(`UPDATE invoices SET language = 'nl' WHERE language IS NULL OR language = ''`).run();
        }
        catch (e) {
            // Ignoruj błędy (np. gdy kolumna już ma wartości)
        }
    }
    ...
    createInvoice(invoiceData) {
      ...
      INSERT INTO invoices (
        ...
        currency, language, status, total_net, total_vat, total_gross, vat_note,
        payment_qr_payload, payment_reference, notes, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, invoiceNumber, invoiceData.company_id || '1', invoiceData.client_id, invoiceData.issue_date, invoiceData.due_date, invoiceData.currency || 'EUR', invoiceData.language || 'nl', invoiceData.status || 'unpaid', invoiceData.total_net || 0, invoiceData.total_vat || 0, invoiceData.total_gross || 0, invoiceData.vat_note || '', invoiceData.payment_qr_payload || '', invoiceData.payment_reference || invoiceNumber, invoiceData.notes || '', now, now);
    ...
    updateInvoice(id, invoiceData) {
        ...
        const stmt = this.db.prepare(`
      UPDATE invoices SET
        client_id = ?, issue_date = ?, due_date = ?, currency = ?, language = ?, status = ?,
        total_net = ?, total_vat = ?, total_gross = ?, vat_note = ?,
        payment_qr_payload = ?, payment_reference = ?, notes = ?, updated_at = ?
      WHERE id = ?
    `);
        stmt.run(invoiceData.client_id, invoiceData.issue_date, invoiceData.due_date, invoiceData.currency, invoiceData.language || 'nl', invoiceData.status, invoiceData.total_net, invoiceData.total_vat, invoiceData.total_gross, invoiceData.vat_note, invoiceData.payment_qr_payload, invoiceData.payment_reference, invoiceData.notes, now, id);
    ...
    // CRUD Operacje - Wydatki (Expenses)
    getExpenses() {
    ...
    createExpense(expenseData) {
      ...
      INSERT INTO expenses (
        id, date, category, supplier, description, amount_net, vat_rate, vat_amount,
        amount_gross, currency, payment_method, is_vat_deductible, is_business_expense,
        private_percentage, project_id, client_id, invoice_number, receipt_url, notes,
        created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, expenseData.date, expenseData.category, expenseData.supplier, expenseData.description || '', expenseData.amount_net || 0, expenseData.vat_rate || 0, expenseData.vat_amount || 0, expenseData.amount_gross || 0, expenseData.currency || 'EUR', expenseData.payment_method || 'bank_transfer', expenseData.is_vat_deductible ? 1 : 0, expenseData.is_business_expense ? 1 : 0, expenseData.private_percentage || 0, expenseData.project_id || null, expenseData.client_id || null, expenseData.invoice_number || '', expenseData.receipt_url || '', expenseData.notes || '', now, now);
        return { id, created_at: now, updated_at: now, ...expenseData };
    }
    updateExpense(id, expenseData) {
        const now = new Date().toISOString();
        const stmt = this.db.prepare(`
      UPDATE expenses SET
        date = ?, category = ?, supplier = ?, description = ?, amount_net = ?,
        vat_rate = ?, vat_amount = ?, amount_gross = ?, currency = ?,
        payment_method = ?, is_vat_deductible = ?, is_business_expense = ?,
        private_percentage = ?, project_id = ?, client_id = ?, invoice_number = ?,
        receipt_url = ?, notes = ?, updated_at = ?
      WHERE id = ?
    `);
        stmt.run(expenseData.date, expenseData.category, expenseData.supplier, expenseData.description || '', expenseData.amount_net || 0, expenseData.vat_rate || 0, expenseData.vat_amount || 0, expenseData.amount_gross || 0, expenseData.currency || 'EUR', expenseData.payment_method || 'bank_transfer', expenseData.is_vat_deductible ? 1 : 0, expenseData.is_business_expense ? 1 : 0, expenseData.private_percentage || 0, expenseData.project_id || null, expenseData.client_id || null, expenseData.invoice_number || '', expenseData.receipt_url || '', expenseData.notes || '', now, id);
        const updatedExpense = this.db.prepare('SELECT * FROM expenses WHERE id = ?').get(id);
    ...
    generateInvoiceNumber() {
        ...
        let nextSeq;
        if (!counter) {
            // Pierwszy numer w tym miesiącu
            nextSeq = 1;
            this.db.prepare('INSERT INTO invoice_counters (year, month, last_seq) VALUES (?, ?, ?)').run(year, month, nextSeq);
        }
        else {
            // Zwiększ licznik
            nextSeq = counter.last_seq + 1;
            this.db.prepare('UPDATE invoice_counters SET last_seq = ? WHERE year = ? AND month = ?').run(nextSeq, year, month);
        }
        return `FV-${year}-${month.toString().padStart(2, '0')}-${nextSeq.toString().padStart(3, '0')}`;
    }
